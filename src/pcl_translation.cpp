...
int main( int argc, char** argv)
{
    ...
    trans_matrix.translation() << 50., 0.0, 0.0;


    // The same rotation matrix as before; theta radians around Z axis
    trans_matrix.rotate (Eigen::AngleAxisf (theta, Eigen::Vector3f::UnitZ()));

    // Executing the transformation
    pcl::PointCloud<Point_t>::Ptr transformed_cloud (new pcl::PointCloud<Point_t> ());
    // You can either apply transform_1 or transform_2; they are the same
    pcl::transformPointCloud (*input_cloud, *transformed_cloud, trans_matrix);
